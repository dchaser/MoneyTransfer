	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (drawableBottom != null
				...
				clickListener.onClick(utilities.DrawableClickListener.DrawablePosition.BOTTOM);
				return super.onTouchEvent(event);
			}

			if (drawableTop != null
					&& drawableTop.getBounds().contains(actionX, actionY)) {
				clickListener.onClick(utilities.DrawableClickListener.DrawablePosition.TOP);
				return super.onTouchEvent(event);
			}
			...
			if (drawableLeft != null) {
				...
				// CoreSystemContext.SCALE is
				// the variable holding device density. use
				// getResources().getDisplayMetrics().density instead of that,
				// you might need to create a static varaible in some other
				// file. This to convert px to dp. A simple solution would be to
				...
				// after if (event.getAction() == MotionEvent.ACTION_DOWN) { in
				// onTouchEvent(MotionEvent event) method so that user can get a
				// click sound.
